...
setwd("/Users/kaylar/Documents/GitHub/dsc520")
...
date_df <- data.frame(date = covid_df$date, year = as.numeric(format(covid_df$date, format = "%Y")),
                      month = as.numeric(format(covid_df$date, format = "%m")), 
                      day = as.numeric(format(covid_df$date, format = "%d")))
covid_df <- cbind(date_df, covid_df)
...
ggplot(data = florida_df, aes(x = date, group=1, color = "darkred")) +
  ...
  scale_colour_manual("", breaks = c("New York", "California", "Florida"), 
